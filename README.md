...
####名寄せについて
EnjoyFishingでは、ひとつの魚名に対し複数のIDについては管理することができるのですが、`ディル=ブラックソール`や`バストアサーディン=ハムシー=センローサーディン` などの様に、ひとつのIDに対し複数の魚名も存在します。  
これらを名寄せルール(RenameFish.xml)に基づき、同一の魚名に名寄せを行って管理します。

### 魚リスト
![魚リスト](http://i.imgur.com/vlkPqbb.png)
...
### 釣り設定
![釣り設定](http://i.imgur.com/vlkPqbb.png)
